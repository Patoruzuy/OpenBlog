...
from backend.models.tag import Tag, PostTag
...
class AdminTagService:
    ...
    def list_tags(*, q: str | None = None, page: int = 1) -> tuple[list[dict], int]:
        ...
        query = (
            select(Tag, func.coalesce(post_count_sq.c.post_count, 0).label("post_count"))
    ...
    @staticmethod
    def create(*, name: str, description: str | None = None, color: str | None = None) -> Tag:
        name = name.strip()
        if not name:
        ...
        tag = Tag(name=name, slug=slug, description=description or None, color=color or None)
        db.session.add(tag)
        db.session.commit()
        return tag

    @staticmethod
    def update(tag: Tag, *, name: str | None = None, description: str | None = None, color: str | None = None) -> Tag:
        if name is not None:
            name = name.strip()
            if not name:
