...
class AdminDashboardService:
    ...
    def get_snapshot() -> dict:
        ...
        active_contributors_30d = db.session.scalar(
            select(func.count(func.distinct(Revision.author_id)))
            .where(Revision.created_at >= since_30d)
        ) or 0

        # ── System health (lightweight, best-effort) ───────────────────────
        from backend.services.system_health_service import SystemHealthService  # noqa: PLC0415
        try:
            health = SystemHealthService.get_status()
        except Exception as exc:
            import logging  # noqa: PLC0415
            logging.getLogger(__name__).warning("SystemHealthService.get_status() failed: %s", exc, exc_info=True)
            health = {"db": {"ok": False, "label": "?"}, "redis": {"ok": False, "label": "?"}, "celery": {"ok": False, "label": "?"}}
        ...
        return {
            ...
            "revisions_recent_pending":  recent_rev_counts.get(RevisionStatus.pending, 0),
            "revisions_recent_accepted": recent_rev_counts.get(RevisionStatus.accepted, 0),
            # users
