...
_DEFAULTS: list[tuple[str, Any, str, str]] = [
    ("site_title",            "OpenBlog",           "general",  "Public site title shown in browser tab and header."),
    ("site_description",      "A collaborative developer blog, open to improvement.",
                                                    "general",  "Homepage meta-description used for SEO and social sharing."),
    ("contact_email",         "",                   "general",  "Public contact email shown in footer."),
    ("footer_copyright",      "",                   "general",  "Custom footer copyright line (leave blank to auto-generate)."),
    ("registration_open",     True,                 "auth",     "Allow new user self-registration."),
    ("require_email_verify",  True,                 "auth",     "Require email verification before users can contribute."),
    ("default_post_status",   "draft",              "content",  "Default status for newly created posts."),
    ...
    ("maintenance_mode",      False,                "ops",      "When true, show a maintenance page to non-admin visitors."),
    ("seo_default_image",     "",                   "seo",      "Default Open Graph image URL used when a post has no cover."),
    ("analytics_enabled",     True,                 "ops",      "Enable server-side page-view analytics collection."),
]
...
class SiteSettingsService:
    ...
    def get_all() -> dict[str, Any]:
        ...
        result = {r.key: json.loads(r.value) if r.value is not None else None for r in rows}
    ...
    def get_all_rows() -> list:
        rows = list(db.session.scalars(select(SiteSetting).order_by(SiteSetting.group, SiteSetting.key)).all())
    ...
    def seed_defaults(actor: User | None = None) -> None:
        ...
        for key, default, group, desc in _DEFAULTS:
            existing = db.session.scalar(select(SiteSetting).where(SiteSetting.key == key))
            if existing is None:
                s = SiteSetting(
                    key=key,
                    value=json.dumps(default),
