...
def author(make_user_token):
    user, tok = make_user_token("analytics_api_author@example.com", "analyticsapiauthor")
    return user, tok


@pytest.fixture()
def editor(make_user_token):
    user, tok = make_user_token("analytics_api_editor@example.com", "analyticsapieditor", role="editor")
    return user, tok


@pytest.fixture()
def admin(make_user_token):
    user, tok = make_user_token("analytics_api_admin@example.com", "analyticsapiadmin", role="admin")
    return user, tok


@pytest.fixture()
def reader(make_user_token):
    user, tok = make_user_token("analytics_api_reader@example.com", "analyticsapireader")
    return user, tok
...
class TestPostAnalytics:
    def test_author_can_access_own_post(self, auth_client, author, pub_post, db_session):
        _, tok = author
        resp = auth_client.get(
            f"/api/posts/{pub_post.slug}/analytics", headers=_h(tok)
        )
    ...
    def test_editor_can_access_any_post(self, auth_client, editor, pub_post, db_session):
        _, tok = editor
        resp = auth_client.get(
            f"/api/posts/{pub_post.slug}/analytics", headers=_h(tok)
        )
...
class TestTopPosts:
    ...
    def test_limit_param(self, auth_client, editor, db_session):
        _, tok = editor
        resp = auth_client.get(
            "/api/analytics/top-posts?limit=5", headers=_h(tok)
        )
        assert resp.status_code == 200
    ...
    def test_days_param(self, auth_client, editor, db_session):
        _, tok = editor
        resp = auth_client.get(
            "/api/analytics/top-posts?days=7", headers=_h(tok)
        )
        assert resp.status_code == 200
    ...
    def test_limit_capped_at_50(self, auth_client, editor, db_session):
        _, tok = editor
        resp = auth_client.get(
            "/api/analytics/top-posts?limit=9999", headers=_h(tok)
        )
    ...
    def test_days_capped_at_365(self, auth_client, editor, db_session):
        _, tok = editor
        resp = auth_client.get(
            "/api/analytics/top-posts?days=9999", headers=_h(tok)
        )
